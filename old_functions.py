"""Retiring this file 12/15/22, there are a few useful swaths of code so not deleting. fka: main.py"""

import random
from base_mortys import *
from Fortuna import random_range, random_value

"""These early instances randomized within themselves, drawing from classes that had set defaults.
Calling them now will result in twice-randomized stats."""
...
def random_list():
    return random_value((random_gotron, random_cubism, random_s4, random_dragon, random_fan,
...
class RandomMorty:
    """strange parameters but gets the job done"""
    ...
    def __str__(self):
        return f"The randomized Morty is a {self.morty_type.name} Morty! {self.morty_type}"
...
def make_random_teams():
    """We've improved on this old function by slicing not looping"""
    team_a = []
    team_b = []
